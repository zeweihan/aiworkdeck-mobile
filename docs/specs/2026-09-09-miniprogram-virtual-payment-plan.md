...
- 道具三档已发布到现网（微信单道具上限 100 元，所以没有 300 档）。**价格单位是元**（后台与 `/xpay/start_upload_goods` 一致），signData 的 `goodsPrice` 才是分；下表「价格（分）」是 signData 口径，后台/接口应录 10 / 50 / 100（2026-09-14 -15013 根因）：
