...
| AVRecorder | 纯音频：`audioSourceType: AUDIO_SOURCE_TYPE_MIC`，`profile: { audioCodec: AUDIO_AAC, fileFormat: CFT_MPEG_4A, audioSampleRate: 44100, audioChannels: 1, audioBitrate: 64000 }`，输出 `.m4a`；支持 `pause()/resume()` 续写同一文件；`on('audioInterrupt')` 回 `audio.InterruptEvent`（`hintType` = `INTERRUPT_HINT_PAUSE` / `INTERRUPT_HINT_RESUME`） |
...
- 中断：`on('audioInterrupt', e)`：`hintType === INTERRUPT_HINT_PAUSE` → `clock.interrupt`、`interrupted = true`（录音机已被系统暂停）；`INTERRUPT_HINT_RESUME` → `recorder.resume()` 续写同一文件，成功则 `clock.resume`、`interrupted = false`。`INTERRUPT_HINT_STOP` 按停止处理（落库）。
